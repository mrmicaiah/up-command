/**
 * Author Tools - Blog author profile management
 */
...
export function registerAuthorTools(server: McpServer, ctx: ToolContext): void {
  const { env } = ctx;
  ...
  server.tool('author_create', {
    ...
    const existing = await env.DB.prepare('SELECT id FROM authors WHERE slug = ?').bind(normalizedSlug).first();
    if (existing) {
  ...
  server.tool('author_update', {
    ...
    const existing = await env.DB.prepare('SELECT * FROM authors WHERE id = ?').bind(id).first();
    if (!existing) {
      return { content: [{ type: 'text', text: 'Author not found.' }] };
    }
    ...
    if (slug !== undefined) {
      const normalizedSlug = slug.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
      const slugCheck = await env.DB.prepare('SELECT id FROM authors WHERE slug = ? AND id != ?').bind(normalizedSlug, id).first();
      if (slugCheck) return { content: [{ type: 'text', text: 'Slug "' + normalizedSlug + '" is already in use.' }] };
      updates.push('slug = ?'); bindings.push(normalizedSlug);
    }
    if (bio !== undefined) { updates.push('bio = ?'); bindings.push(bio || null); }
    if (photo_url !== undefined) { updates.push('photo_url = ?'); bindings.push(photo_url || null); }
    if (email !== undefined) { updates.push('email = ?'); bindings.push(email || null); }
    
    if (updates.length === 0) return { content: [{ type: 'text', text: 'No updates provided.' }] };
    
    bindings.push(id);
    await env.DB.prepare('UPDATE authors SET ' + updates.join(', ') + ' WHERE id = ?').bind(...bindings).run();
  ...
  server.tool('author_delete', {
    ...
    const existing = await env.DB.prepare('SELECT name FROM authors WHERE id = ?').bind(id).first();
    if (!existing) return { content: [{ type: 'text', text: 'Author not found.' }] };
